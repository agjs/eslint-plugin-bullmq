...
function getReceiverKey(node) {
  ...
  if (node.type === AST_NODE_TYPES.MemberExpression && node.object.type === AST_NODE_TYPES.ThisExpression && node.property.type === AST_NODE_TYPES.Identifier) {
...
function findEnclosingFunctionOrClass(node) {
  let current = node.parent;
  while (current) {
    if (current.type === AST_NODE_TYPES.FunctionDeclaration || current.type === AST_NODE_TYPES.FunctionExpression || current.type === AST_NODE_TYPES.ArrowFunctionExpression || current.type === AST_NODE_TYPES.ClassDeclaration || current.type === AST_NODE_TYPES.ClassExpression) {
      return current;
    }
    current = current.parent;
  }
...
function findObjectProperty(obj, name) {
  for (const property of obj.properties) {
    ...
    if (property.key.type === AST_NODE_TYPES.Identifier && property.key.name === name) {
      return property;
    }
    if (property.key.type === AST_NODE_TYPES.Literal && property.key.value === name) {
      return property;
    }
  }
  return null;
  ...
  create(context, [options]) {
    ...
    return {
      ...
      CallExpression(node) {
        ...
        if (nameArg.type === AST_NODE_TYPES2.Literal && typeof nameArg.value === "string") {
          context.report({
            node: nameArg,
        ...
        if (nameArg.type === AST_NODE_TYPES2.TemplateLiteral && nameArg.expressions.length === 0 && nameArg.quasis.length > 0) {
          context.report({
            node: nameArg,
            messageId: "literalJobName",
...
function isAttemptsLiteralOne(value) {
  return value.type === AST_NODE_TYPES3.Literal && typeof value.value === "number" && value.value === 1;
}
  ...
  create(context) {
    ...
    return {
      ...
      NewExpression(node) {
        ...
        if (value.type === AST_NODE_TYPES4.Literal && typeof value.value === "number" && value.value <= 0) {
          context.report({
            node: value,
        ...
        if (value.type === AST_NODE_TYPES4.UnaryExpression && value.operator === "-" && value.argument.type === AST_NODE_TYPES4.Literal && typeof value.argument.value === "number" && value.argument.value > 0) {
          context.report({
            node: value,
            messageId: "invalidConcurrency",
...
function classOwnsWorker(cls, imports) {
  for (const member of cls.body.body) {
    if (member.type === AST_NODE_TYPES5.PropertyDefinition && member.value && isNewWorker(member.value, imports)) {
      return true;
    }
    if (member.type === AST_NODE_TYPES5.MethodDefinition) {
      ...
      const found = walkSome(fnBody, (n) => {
        if (n.type !== AST_NODE_TYPES5.AssignmentExpression) {
          return false;
        }
        if (n.left.type !== AST_NODE_TYPES5.MemberExpression || n.left.object.type !== AST_NODE_TYPES5.ThisExpression) {
          return false;
        }
...
function getMethodName(method) {
  if (method.key.type === AST_NODE_TYPES5.Identifier) {
    return method.key.name;
  }
  if (method.key.type === AST_NODE_TYPES5.Literal && typeof method.key.value === "string") {
...
function collectOnListeners(program) {
  ...
  walkAll(program, (node) => {
    if (node.type !== AST_NODE_TYPES6.CallExpression) {
      return;
    }
    if (node.callee.type !== AST_NODE_TYPES6.MemberExpression || node.callee.property.type !== AST_NODE_TYPES6.Identifier || node.callee.property.name !== "on") {
      return;
    }
    ...
    if (!eventArg || eventArg.type !== AST_NODE_TYPES6.Literal || typeof eventArg.value !== "string") {
      return;
    }
